...
appControllers.controller('PubblicationCtrl', [ '$scope', "$route", 'adminAPIservice', '$translate', '$interval' , '$timeout',
...
//				}
	...
	$scope.enableExecuteButton = function(){
		...
		return( "result", $scope.tenantForDataset && $scope.projectCode && $scope.operationCode && $scope.datasetsStr && $scope.writemodeSelect);
	}
	...
	$scope.writeMode  = Constants.PUBBLICATION_WRITE_MODE;
	
	
	$scope. callOoziePubblication = function(){
		...
		$scope.datasetsToUpdateStr = datasetsToUpdate.join();
		  ...
		  var toWrite  = $scope.writemodeSelect;
		  var makeHiveToCsv = 0;
		  if ($scope.flgCreateCsv) makeHiveToCsv = 1;
		  ...
		  var actionOozieRequest = {
			...
			adminAPIservice.callOoziePromotion(actionOozieRequest).success(function(response) {
				console.info("callOoziePubblication - response",response);	
				...
				console.error("callOoziePubblication ERROR ",response);	
			});
		}
