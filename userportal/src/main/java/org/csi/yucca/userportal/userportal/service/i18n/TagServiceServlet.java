...
@WebServlet(description = "Configuration language TAGS for clients", urlPatterns = { "/api/tags" }, initParams = { @WebInitParam(name = "language", value = "it"), }, asyncSupported = true)
public class TagServiceServlet extends I18nServlet {
	...
	protected JSONArray loadMessages(Locale currentLocale) throws MalformedURLException {
		...
		try {
			...
			is = new URL(config.getProperty(Config.API_ADMIN_URL_KEY) + "/1/public/tags?ecosystemCode=SDNET").openStream();
	...
	protected String formatMessages(JSONArray elements, Locale locale) {
		...
		try {
			...
			for(int i = 0 ; i < elements.length() ; i++){
				String tagCode = elements.getJSONObject(i).getString("tagcode");
